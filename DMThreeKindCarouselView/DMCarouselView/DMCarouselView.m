...
@interface DMCarouselView () <UIScrollViewDelegate>
{
    //记录当前图片下标
...
-(instancetype)initWithFrame:(CGRect)frame andScrollViewMode:(ScrollViewMode)scrollViewMode andImagesArray:(NSArray *)imagesArray
{
    ...
    if (self)
    {
        ...
        [self initViewWithImagesArray:imagesArray];
    }
    return self;
}
...
- (void)addImagesArray:(NSArray *)imagesArray centerItemFrame:(CGRect)centerItemFrame currentImageClick:(CurrentImageClick)currentImageClickBlock
{
    ...
    //如果图片数组的个数 <= 1 ，则只添加一张图片，并且返回
    if (imagesArray.count <= 1) {
        [self addNextImageWith:self.firstImageView WithImageIndex:0];
        return;
    }
...
//初始化视图
- (void)initViewWithImagesArray:(NSArray *)imagesArray
{
    //初始化当前图片(默认下标为0);
